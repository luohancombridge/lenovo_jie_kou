            ...
            if self.data0!=self.data:
              for k,i in enumerate(self.data):
                ...
                # 判断是否有包含#的字段若果则执行python代码
                    for k,i in enumerate(self.data):
                        if '##' in i:
                            self.data[k]=excel_data_exe.han_shu(i)
                # 生成json字符串
    ...
    def  reque(self,data,req):
        if req[2].strip()=='application/json':
           ...
           return response.read()
            ...
            request = urllib2.Request(req[0], data)
            s=[i.replace('\n','')  for i in open(os.path.join(self.path, 'public.txt')).readlines()]
            head=add_header(s[0],s[1],data)
            [reques.add_header(i,head[i]) for i in head.keys()]
...
class log(object):
    def __init__(self):
        ...
        fh = logging.FileHandler(r'C:\work\lenove_jie_kou\log.txt')
...
if __name__=='__main__':
    create_stock(r'C:\work\lenove_jie_kou\queryPriceDetail')
